...
export default function ProgramEdit({ program }: { program: any }) {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Edit Program SASANA" />
            ...
            <div className="p-6 md:p-8 space-y-8 bg-slate-50/50 min-h-full">
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900">Edit Program SASANA</h1>
                        <p className="text-slate-500 text-sm">Perbarui data program atau kegiatan desa.</p>
                    </div>
                </div>
                ...
                <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 md:p-8 max-w-4xl">
                    <form onSubmit={submit} encType="multipart/form-data" className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <Textarea id="content" value={data.content} onChange={e => setData('content', e.target.value)} rows={6} />
                        </div>
                        ...
                        {program.image_path && (
                            <div className="space-y-2">
                                ...
                                <img src={`/storage/${program.image_path}`} alt={program.title} className="h-32 rounded-lg object-cover border border-slate-200" />
                            </div>
                        )}
